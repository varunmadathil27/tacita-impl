...
pub struct ProverPreprocessing {
    pub(crate) n: usize, //size of the committee as a power of 2
    pub(crate) pks: Vec<G1>, //g^sk_i for each party i
    pub(crate) q1_coms : Vec<G1>, //preprocessed contributions for pssk_q1
    pub(crate) q2_coms : Vec<G1>, //preprocessed contributions for pssk_q2
}
...
pub fn run_demo() {
    ...
    for i in 0..bitmap.len() {
        ...
        if bitmap[i] == F::from(1) {
            ...
            let signature = signer::sign(&params, &pp, &sk[i], &vp.h_0, &message, i);
            let duration = start.elapsed();
            total_duration += duration;
            signatures.push((i, signature)); // Store the index and signature
        }
        else{
            let empty_signature = Signature {
                M_i: get_zero_poly_com_g1(&params),        // Zero element in G1
                sig_i: get_zero_poly_com_g2(&params),        // Zero element in G2
                shint_i: vec![],          // Empty vector
                skshint_i: vec![],        // Empty vector
            };
            signatures.push((i, empty_signature));
        }
    }
    println!("Average Time taken for signing when n = {} is: {:?}", n, total_duration/counter);
    // println!("{:?}", bitmap);
    println!("Signatures: {:?}", signatures.len());

    match compute_signature_size(&signatures[5].1) {
    ...
    let π = prove(&params, &pp, &cache, &weights, &bitmap,&signatures);
    let duration = start.elapsed();
    println!("Time elapsed in prover is: {:?}", duration);

    // Compute and print the size of the proof
    match compute_proof_size(&π) {
...
) -> (VerifierPreprocessing, ProverPreprocessing)
{
    ...
    for (pk_i, com_sk_l_i_1, com_sk_l_i_2, q1_i, q2_i) in all_parties_setup {
        q1_contributions.push(q1_i.clone());
        q2_contributions.push(q2_i.clone());
        pks.push(pk_i.clone());
        com_sks.push(com_sk_l_i_2.clone());
    }
...
// /// Function to compute the size of a signature in kilobytes
// fn compute_signature_size(signature: &Signature) -> Result<f64, SerializationError> {
//     let mut serialized_signature = Vec::new();
//     signature.serialize_compressed(&mut serialized_signature)?; // Serialize the signature
//     let size_in_kb = serialized_signature.len() as f64 / 1024.0; // Convert size to kilobytes
//     Ok(size_in_kb)
// }
...
pub fn prove(
    ...
    let mut s_q1_sums = vec![];
    let mut s_q2_sums = vec![];
    let mut sks_q1_sums = vec![];
    let mut sks_q2_sums = vec![];

    // Iterate over the signatures and extract the q1_sum element from shint_i
    for (_, signature) in signatures.iter() {
        if signature.shint_i.len() > 3 {
            s_q2_sums.push(signature.shint_i[3]); // Add the q2_sum element (4th element in shint_i)
            s_q1_sums.push(signature.shint_i[2]); // Add the q1_sum element (4th element in shint_i)
        } else {
            s_q2_sums.push(get_zero_poly_com_g1(&params));
            s_q1_sums.push(get_zero_poly_com_g1(&params)); // Add a zero element if not enough elements
            // panic!("shint_i does not have enough elements to extract q1_sum");
        }

        if signature.skshint_i.len() > 3 {
            sks_q2_sums.push(signature.skshint_i[3]); // Add the q2_sum element (4th element in skshint_i)
            sks_q1_sums.push(signature.skshint_i[2]); // Add the q1_sum element (4th element in skshint_i)
        } else {
            sks_q2_sums.push(get_zero_poly_com_g1(&params));
            sks_q1_sums.push(get_zero_poly_com_g1(&params));
        }
    }

    s_q2_sums.push(get_zero_poly_com_g1(&params));
    s_q1_sums.push(get_zero_poly_com_g1(&params));
    sks_q2_sums.push(get_zero_poly_com_g1(&params));
    sks_q1_sums.push(get_zero_poly_com_g1(&params));
    ...
    let s_q1_com = filter_and_add(&params, &s_q1_sums, &bitmap);
    let s_q2_com = filter_and_add(&params, &s_q2_sums, &bitmap);
    let sk_s_q1_com = filter_and_add(&params, &sks_q1_sums, &bitmap);
    let sk_s_q2_com = filter_and_add(&params, &sks_q2_sums, &bitmap);
...
fn compute_signature_size(signature: &Signature) -> Result<f64, SerializationError> {
    ...
    // Serialize each field of the signature
    signature.M_i.serialize_compressed(&mut serialized_signature)?;
    signature.sig_i.serialize_compressed(&mut serialized_signature)?;
    for shint in &signature.shint_i {
        shint.serialize_compressed(&mut serialized_signature)?;
    }
    for skshint in &signature.skshint_i {
        skshint.serialize_compressed(&mut serialized_signature)?;
    }
...
/// Function to compute the size of a proof in kilobytes
fn compute_proof_size(proof: &Proof) -> Result<f64, SerializationError> {
    let mut serialized_proof = Vec::new();

    // Serialize each field of the proof
    ...
    proof.psw_of_r_div_ω.serialize_compressed(&mut serialized_proof)?;
    proof.psw_of_r_div_ω_proof.serialize_compressed(&mut serialized_proof)?;
    ...
    proof.psw_check_q_of_r.serialize_compressed(&mut serialized_proof)?;
    ...
    proof.psw_wff_q_of_x_com.serialize_compressed(&mut serialized_proof)?;
    proof.psw_check_q_of_x_com.serialize_compressed(&mut serialized_proof)?;
    proof.b_wff_q_of_x_com.serialize_compressed(&mut serialized_proof)?;
    proof.b_check_q_of_x_com.serialize_compressed(&mut serialized_proof)?;
    proof.sk_q1_com.serialize_compressed(&mut serialized_proof)?;
    proof.sk_q2_com.serialize_compressed(&mut serialized_proof)?;

    // Compute the size in kilobytes
    let size_in_kb = serialized_proof.len() as f64 / 1024.0;
    Ok(size_in_kb)
}
...
pub(crate) fn get_zero_poly_com_g1(params: &UniversalParams<Bls12_381>) -> G1 {
...
pub(crate) fn party_i_setup_material(
    params: &UniversalParams<Bls12_381>,
    n: usize, 
    i: usize, 
    sk_i: &F) -> (G1, G1, G2, Vec<G1>, G1) {
    ...
    (pk, com_sk_l_i_1, com_sk_l_i_2, q1_material, q2_com)
}
...
mod tests {
    ...
    fn compute_pssk_q1_poly(
        ...
        let n = sk.len();
        let z_of_x = utils::compute_vanishing_poly(n as usize);
        //Li(x) · Li(x) − Li(x) / Z(x)
